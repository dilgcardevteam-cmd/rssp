...
<div class="py-6 space-y-6 max-w-6xl mx-auto font-montserrat">
    ...
    <div class="rounded-xl border <?php echo e($backupReminder['is_overdue'] ? 'border-amber-200 bg-amber-50' : 'border-emerald-200 bg-emerald-50'); ?> px-4 py-3 relative z-40">
        <div class="flex items-start gap-3">
            <span class="mt-0.5 inline-flex h-6 w-6 items-center justify-center rounded-full <?php echo e($backupReminder['is_overdue'] ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'); ?>">
                <i data-feather="<?php echo e($backupReminder['is_overdue'] ? 'alert-triangle' : 'shield'); ?>" class="w-4 h-4"></i>
            </span>
            <div class="text-sm">
                <p class="font-semibold <?php echo e($backupReminder['is_overdue'] ? 'text-amber-800' : 'text-emerald-800'); ?>">
                    <?php echo e($backupReminder['status_label']); ?>

                </p>
                <p class="<?php echo e($backupReminder['is_overdue'] ? 'text-amber-700' : 'text-emerald-700'); ?>">
                    <?php echo e($backupReminder['reminder_message']); ?>

                </p>
                <p class="mt-1 text-xs text-slate-600">
                    Last successful backup:
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(!empty($backupReminder['latest_backup_at'])): ?>
                        <?php echo e(\Carbon\Carbon::parse($backupReminder['latest_backup_at'])->format('F j, Y g:i A')); ?>

                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(!is_null($backupReminder['days_since_last_backup'])): ?>
                            (<?php echo e((int) $backupReminder['days_since_last_backup']); ?> day(s) ago)
    ...
    <div class="flex gap-2 overflow-x-auto pb-1">
        <button
            type="button"
            class="utility-tab inline-flex shrink-0 items-center gap-2 rounded-full border px-4 py-2 text-sm font-semibold transition <?php echo e($activeUtilityTab === 'backupRestorePanel' ? 'border-[#0D2B70] bg-[#0D2B70] text-white shadow-sm' : 'border-blue-200 bg-blue-50 text-[#0D2B70] hover:bg-blue-100'); ?>"
            ...
            aria-selected="<?php echo e($activeUtilityTab === 'backupRestorePanel' ? 'true' : 'false'); ?>"
        >
        ...
        <button
            type="button"
            class="utility-tab inline-flex shrink-0 items-center gap-2 rounded-full border px-4 py-2 text-sm font-semibold transition <?php echo e($activeUtilityTab === 'schedulerPanel' ? 'border-emerald-700 bg-emerald-700 text-white shadow-sm' : 'border-emerald-200 bg-emerald-50 text-emerald-700 hover:bg-emerald-100'); ?>"
            ...
            aria-selected="<?php echo e($activeUtilityTab === 'schedulerPanel' ? 'true' : 'false'); ?>"
        >
    ...
    <section id="backupRestorePanel" class="utility-tab-panel space-y-6 <?php echo e($activeUtilityTab === 'backupRestorePanel' ? '' : 'hidden'); ?>">
    <div class="grid gap-6 lg:grid-cols-2">
        <div class="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
            ...
            <div class="p-6 space-y-4">
                ...
                <div class="rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-600 space-y-1">
                    <p><span class="font-semibold text-slate-700">Database:</span> <?php echo e($databaseName ?? config('database.connections.' . config('database.default') . '.database')); ?></p>
                    <p><span class="font-semibold text-slate-700">Host:</span> <?php echo e($databaseHost ?? config('database.connections.' . config('database.default') . '.host')); ?></p>
                    <p><span class="font-semibold text-slate-700">Next Scheduled Run:</span> <?php echo e($nextScheduledRun ?? 'Scheduler disabled'); ?></p>
                </div>
                <form method="POST" action="<?php echo e(route('admin.backup.run')); ?>" class="flex flex-col gap-3 sm:flex-row sm:items-center">
                    <?php echo csrf_field(); ?>
        ...
        <div class="rounded-2xl border border-red-200 bg-white shadow-sm overflow-hidden">
            ...
            <div class="p-6 space-y-4">
                ...
                <form id="restoreDatabaseForm" method="POST" action="<?php echo e(route('admin.backup.restore')); ?>" enctype="multipart/form-data" class="space-y-4">
                    <?php echo csrf_field(); ?>
                    <div>
    ...
    <section id="schedulerPanel" class="utility-tab-panel space-y-6 <?php echo e($activeUtilityTab === 'schedulerPanel' ? '' : 'hidden'); ?>">
    <section id="scheduled-backup" class="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        <div class="px-6 py-5 border-b border-slate-100 bg-gradient-to-r from-emerald-50 via-white to-[#EAF2FF]">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                ...
                <span class="inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-semibold <?php echo e(($automationSetting?->is_enabled) ? 'border border-emerald-100 bg-emerald-50 text-emerald-700' : 'border border-slate-200 bg-slate-100 text-slate-600'); ?>">
                    <span class="h-2 w-2 rounded-full <?php echo e(($automationSetting?->is_enabled) ? 'bg-emerald-500' : 'bg-slate-400'); ?>"></span>
                    <?php echo e(($automationSetting?->is_enabled) ? 'Scheduler Enabled' : 'Scheduler Disabled'); ?>
        ...
        <div class="p-6 space-y-6">
            <form method="POST" action="<?php echo e(route('admin.backup.schedule')); ?>" class="grid gap-4 md:grid-cols-2">
                <?php echo csrf_field(); ?>
                ...
                <div class="md:col-span-2 rounded-xl border border-blue-200 bg-blue-50 px-4 py-3">
                    <label for="is_enabled" class="flex items-center gap-3 text-sm font-semibold text-blue-900">
                        <input id="is_enabled" name="is_enabled" type="checkbox" value="1" <?php if($schedulerState['is_enabled']): echo 'checked'; endif; ?> class="h-4 w-4 rounded border-slate-300 text-[#0D2B70] focus:ring-[#0D2B70]">
                ...
                <div>
                    ...
                    <select id="frequency" name="frequency" class="mt-2 block w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm text-slate-700 focus:border-[#0D2B70] focus:outline-none focus:ring-2 focus:ring-[#0D2B70]/20">
                        <option value="daily" <?php if($schedulerState['frequency'] === 'daily'): echo 'selected'; endif; ?>>Daily</option>
                        <option value="weekly" <?php if($schedulerState['frequency'] === 'weekly'): echo 'selected'; endif; ?>>Weekly</option>
                    </select>
                </div>

                <div id="weeklyDayContainer" class="<?php echo e($schedulerState['frequency'] === 'weekly' ? '' : 'hidden'); ?>">
                    ...
                    <select id="weekly_day" name="weekly_day" class="mt-2 block w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm text-slate-700 focus:border-[#0D2B70] focus:outline-none focus:ring-2 focus:ring-[#0D2B70]/20">
                        ...
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $dayOptions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $dayValue => $dayLabel): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoop($loop->index); ?><?php endif; ?>
                            <option value="<?php echo e($dayValue); ?>" <?php if((string) $schedulerState['weekly_day'] === (string) $dayValue): echo 'selected'; endif; ?>><?php echo e($dayLabel); ?></option>
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::endLoop(); ?><?php endif; ?><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::closeLoop(); ?><?php endif; ?>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input id="run_time" name="run_time" type="time" value="<?php echo e($schedulerState['run_time']); ?>"
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="recipient_emails" name="recipient_emails" rows="3" placeholder="name@example.com, admin@example.com"
                        class="mt-2 block w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm text-slate-700 focus:border-[#0D2B70] focus:outline-none focus:ring-2 focus:ring-[#0D2B70]/20"><?php echo e($schedulerState['recipient_emails']); ?></textarea>
            ...
            <form id="testBackupNowForm" method="POST" action="<?php echo e(route('admin.backup.test')); ?>">
                <?php echo csrf_field(); ?>
            </form>
            ...
            <div class="grid gap-4 md:grid-cols-3">
                <div class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
                    ...
                    <p class="mt-2 text-sm font-semibold text-slate-800"><?php echo e($automationSetting?->last_run_at?->format('F j, Y g:i A') ?? 'Not yet run'); ?></p>
                </div>
                <div class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
                    <p class="text-xs font-semibold uppercase tracking-wide text-slate-500">Last Status</p>
                    <p class="mt-2 text-sm font-semibold <?php echo e($automationSetting?->last_status === 'success' ? 'text-emerald-700' : (($automationSetting?->last_status === 'failed') ? 'text-red-700' : 'text-slate-800')); ?>">
                        <?php echo e($automationSetting?->last_status ? ucfirst($automationSetting->last_status) : 'No status yet'); ?>

                    </p>
                </div>
                <div class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
                    <p class="text-xs font-semibold uppercase tracking-wide text-slate-500">Next Run</p>
                    <p class="mt-2 text-sm font-semibold text-slate-800"><?php echo e($nextScheduledRun ?? 'Scheduler disabled'); ?></p>
                </div>
            </div>

            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($automationSetting?->last_error): ?>
                <div class="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
                    <span class="font-semibold">Last scheduler error:</span> <?php echo e($automationSetting->last_error); ?>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-[980px] w-full text-sm table-fixed">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $recentBackupRuns; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $run): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoop($loop->index); ?><?php endif; ?>
                            <tr class="align-top">
                                <td class="px-4 py-3 text-slate-700 break-words"><?php echo e($run->started_at?->format('M j, Y g:i A') ?? '-'); ?></td>
                                <td class="px-4 py-3 text-slate-700 break-words"><?php echo e(ucfirst($run->backup_type)); ?></td>
                                <td class="px-4 py-3">
                                    <span class="inline-flex rounded-full px-2.5 py-1 text-xs font-semibold <?php echo e($run->status === 'success' ? 'bg-emerald-50 text-emerald-700' : ($run->status === 'failed' ? 'bg-red-50 text-red-700' : 'bg-sky-50 text-sky-700')); ?>">
                                        <?php echo e(ucfirst($run->status)); ?>

                                    </span>
                                </td>
                                <td class="px-4 py-3 text-slate-700 break-words"><?php echo e($run->filename ?? '-'); ?></td>
                                <td class="px-4 py-3 text-slate-700 break-words"><?php echo e($run->mailed_to ? implode(', ', $run->mailed_to) : '-'); ?></td>
                                <td class="px-4 py-3 text-slate-500 break-words">
                                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($run->error_message): ?>
                                        <div class="max-h-28 overflow-y-auto pr-1 text-xs leading-5 whitespace-normal break-words">
                                            <?php echo e($run->error_message); ?>
    ...
    (function backupRestorePage() {
        if (typeof window.showAppToast === 'function') {
            <?php if(session('success')): ?>
                window.showAppToast(<?php echo json_encode(session('success'), 15, 512) ?>, 'success');
            <?php endif; ?>

            <?php if(session('error')): ?>
                window.showAppToast(<?php echo json_encode(session('error'), 15, 512) ?>, 'warning');
            <?php endif; ?>
        }
...
<?php echo $__env->make('layout.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\DILG-CAR\resources\views/admin/backup_restore.blade.php ENDPATH**/ ?>